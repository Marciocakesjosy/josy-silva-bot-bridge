...
function extractMessageSource(payload) {
  const conversation = payload.conversa && typeof payload.conversa === "object" ? payload.conversa : null;
  const messageObject = payload.mensagem && typeof payload.mensagem === "object" ? payload.mensagem : null;
  const directMessageSource =
    messageObject?.origem ||
    payload.message?.origem ||
...
function attachReceiptToReply(result, baseUrl) {
  ...
  const isQuotedResult =
    Boolean(result?.ok) &&
    typeof result?.status === "string" &&
    result.status.startsWith("quoted");
  const needsHumanReview = quotePayload?.timing?.confirmationAllowed === false;
  const appendedMessage =
    isQuotedResult
      ? [
          ...
          ...(needsHumanReview
            ? [`- Este pedido precisa de confirmacao humana com pelo menos ${settings.minimumLeadDays} dias de antecedencia`]
            : []),
        ].join("\n")
